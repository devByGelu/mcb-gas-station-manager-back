  ...
  return new Promise(async (resolve, reject) => {
    ...
    if (expenses.length)
      expenses.forEach(async (el) => {
        el.expense.fId = fId
        await sql.query(`INSERT INTO expense SET ?`, el.expense)
      })
...
const getPumpInfo = (num, currentBasis, previousBasis) => {
    ...
    if (currentBasis.length)
      currentBasis.forEach((curr) => {
        if (previousBasis)
          ...
          if (parseInt(curr.pumpNum) === parseInt(num)) {
            const { end, cal, mgn, advance_reading, pName } = curr
            pumpInfo[pName] = {
              end,
              cal,
  ...
  // When trying to create a form
    ...
    previousBasis.forEach((prev) => {
      if (parseInt(prev.pumpNum) === parseInt(num)) {
...
async function formatShiftFormResults(result, shiftFormDate, placement, shift) {
  ...
  if (result.shiftFormNotFound) {
    ...
    let pump4
    const d = new Date(shiftFormDate) // deduct 1 to respect month index
    pump1 = updatePumpInfo(
      getPumpInfo(1, null, result.res),
      pumpInfoDefaults,
      1
    )
    pump2 = updatePumpInfo(
      getPumpInfo(2, null, result.res),
      pumpInfoDefaults,
      2
    )
    pump3 = updatePumpInfo(
      getPumpInfo(3, null, result.res),
      pumpInfoDefaults,
      3
    )
    pump4 = updatePumpInfo(
      getPumpInfo(4, null, result.res),
      pumpInfoDefaults,
      4
    )
    let formData = {
    ...
    if (!currShiftForm.length) {
      ...
      if (!lastShiftForm.length) {
        ...
        if (!lastShiftForm) {
          ...
          resolve(formatShiftFormResults(undefined, date, placement, shift))
        }
      ...
      if (lastShiftForm) {
        ...
        resolve(formatShiftFormResults(result, date, placement, shift))
      }
